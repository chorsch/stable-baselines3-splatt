...
class ActorCriticPolicySplatt(BasePolicy):
    ...
    def __init__(
        ...
        if self.share_features_extractor:
            ...
            self.pi_features_extractor = self.features_extractor
            self.vf_features_extractor = MinigridFeaturesExtractor(self.observation_space, self.features_dim)
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        if self.ortho_init:
            ...
            print(self.share_features_extractor)
            for module, gain in module_gains.items():
                module.apply(partial(self.init_weights, gain=gain))
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        if self.share_features_extractor:
            ...
            pi_features, vf_features = features
            latent_pi = self.mlp_extractor.forward_actor(pi_features)
            #assert False, f'{len(features)}, {features}'
            latent_vf = self.mlp_extractor.forward_critic(vf_features)
        # Evaluate the values for the given observations
        values = self.value_net(latent_vf)
        distribution = self._get_action_dist_from_latent(latent_pi)
